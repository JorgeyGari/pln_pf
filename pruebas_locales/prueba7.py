...
def evaluate_truthfulness(statement):
    ...
    # Detectar directamente verdadero o falso con mayor precisión
    respuesta_limpia = generated_response.strip().lower()
    if any(negacion in respuesta_limpia for negacion in ["no es verdadera", "es falsa", "falso"]):
        is_truthful = False
    elif any(afirmacion in respuesta_limpia for afirmacion in ["es verdadera", "es cierto", "verdadero"]):
        is_truthful = True
    else:
        is_truthful = False  # Por defecto, asumir falso si no está claro
